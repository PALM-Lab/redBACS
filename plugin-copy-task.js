 ...
 * version 0.3
 *
 * William Ngiam and Mindaugas Jurgelis
 *  (modified from Andre Sahakian 
 *    (modified from Chris Jungerius 
 ...
 *  - WN added version and data info to update for jsPsych v9
 *  - WN changed 'trial_data' to 'data' in line with jsPsych data management
 * 
 * TO DO:
 *
 **/
...
var jsPsychCopyingTask = (function (jspsych) {
  ...
  class CopyingTaskPlugin {
    ...
    ///////////////////////////////////////////////////////////////////////
      /* SECTION 3: Add occluding panels (MKJ) */

      var left_panel_params = {
        left: 0,
        top: 0,
        width: (mid_border_left),
        height: canvas_height,
        fill: "grey",
        opacity: 0,
        hasControls: false,
      ...
      var right_panel_params = {
        left: mid_border_right,
        top: 0,
        width: (canvas_width-mid_border_left), // WN: shouldn't this be mid_border_right?
        height: canvas_height,
        fill: "grey",
        opacity: 1,
        hasControls: false,
      ...
      function sendToResource(item) {
        ...
        // Ensures the occluder renders over the item (added by WN)
        canvas.setActiveObject(placeholder_object); 
      ...
      // The functions 2 add panels (MKJ)
      // draw left, occlude right
      function drawLeftPanel(params) {
        var left_occluder = new fabric.Rect(params)

        canvas.add(left_occluder)
        
        left_occluder.on("mouseover", occlude_right)

      }

      function occlude_right() {
        let rp = canvas.getObjects().filter(obj => obj.id ==="right_panel")[0]
        rp.opacity = 1;
    
        let lp = canvas.getObjects().filter(obj => obj.id ==="left_panel")[0]
        lp.opacity = 0;
    
        canvas.sendToBack(lp)
        canvas.bringToFront(rp)

        var event = {
          event: "model grid visible",
          timestamp: Math.round(performance.now()),
        };
        trial_events.push(event);
      ...
      // draw right, occlude left
      function drawRightPanel(params){
        var right_occluder = new fabric.Rect(params)

        // Occlude the resource grid until mouse moves towards right side
        canvas.setActiveObject(right_occluder)
        right_occluder.hasBorders = false
        right_occluder.hasControls = false
        canvas.add(right_occluder)

        right_occluder.on("mouseover", occlude_left)
        
      }
    
      function occlude_left(){
        // This will deactivate the right occluder panel
        canvas.setActiveObject(placeholder_object)
        ...
        canvas.bringToFront(lp)
        canvas.sendToBack(rp)

        var event = {
          event: "resource grid visible",
          timestamp: Math.round(performance.now()),
        };
        trial_events.push(event);
      ...
      const end_trial = () => {
        ...
        var data = {
          ...
          model_grid: getGridContent(model_grid),
          workspace_grid: getGridContent(workspace_grid),
          resource_grid: getGridContent(resource_grid),
          //grid_contents: {
